...
export async function setProductImageAsMain({
  ...
  if (fetchError) {
    throw new Error(`Failed to load product images: ${fetchError.message}`);
  }

  const imageExists = images.some((image) => image.id === imageId);

  if (!imageExists) {
    throw new Error("The selected product image could not be found.");
  }
...
type ReorderProductImagesParams = {
  supabase: ReturnType<typeof import("@/lib/supabase/client").createClient>;
  productId: string;
  imageIds: string[];
};
...
export async function reorderProductImages({
  ...
  productId,
  imageIds,
}: ReorderProductImagesParams): Promise<void> {
